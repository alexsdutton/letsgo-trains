...
class GtkPoweredUpController(GtkController, Gtk.Box):
    def __init__(self, controller: PoweredUpController):
        ...
        self.view.append_column(
            Gtk.TreeViewColumn(
                "Name", Gtk.CellRendererText(editable=True), text=_HubFields.Name
            )
        )
        self.view.append_column(
            Gtk.TreeViewColumn(
                "Battery", Gtk.CellRendererProgress(), value=_HubFields.BatteryLevel
            )
        )
        ...
        self.view.append_column(
            Gtk.TreeViewColumn(
                "Connected",
                Gtk.CellRendererToggle(radio=False),
                active=_HubFields.Connected,
            )
        )
        self.pack_start(self.view, True, True, 0)

        color_cell_renderer = Gtk.CellRendererCombo(
            editable=True, model=self.color_model, text_column=1, has_entry=False
        )
        self.view.append_column(
        ...
        for hub_config in self.controller.hubs.values():
            self.on_hub_discovered(
                self.controller, hub=hub_config.hub, hub_config=hub_config
            )
    ...
    def on_hub_discovered(self, sender, hub: Hub, hub_config: HubConfig):
        self.model.append(
            [
                ...
                hub.battery_level,
                hub_config.active,
                hub_config.connected,
                hub_config.train,
                color_names.get(hub_config.color, ""),
            ]
        )
    ...
    def on_active_toggled(self, widget, path):
        ...
        self.model[path][int(_HubFields.Active)] = not self.model[path][
            _HubFields.Active
        ]
        self.model[path][_HubFields.HubConfig].active = self.model[path][
            _HubFields.Active
        ]

    def on_color_changed(self, widget, path, new_iter):
        self.model[path][int(_HubFields.Color)] = color_names[
            self.color_model[new_iter][0]
        ]
        self.model[path][_HubFields.HubConfig].color = ColorNo(
            self.color_model[new_iter][0]
        )
