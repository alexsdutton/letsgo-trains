...
class BasePoints(Piece):
    ...
    def draw_rails_path(self, cr: cairo.Context, anchor_name: str):
        ...
        if anchor_name == 'out':
            ...
            cr.curve_to(*self.control_points[0],
    ...
    def point_position(self, in_anchor, offset, out_anchor=None):
        ...
        if in_anchor == 'in' and out_anchor == 'branch':
            t = self.intermediate_branch_t[max(0, min(int(offset / self.branch_length * 100), 99))]
            ...
            return x, y, theta
        if in_anchor == 'branch':
            t = self.intermediate_branch_t[max(0, min(99 - int(offset / self.branch_length * 100), 99))]
            x, y = self.branch_bezier(t)
