...
class TophamHatt:
    ...
    def route_train(self, train):
        ...
        while True:
            try:
                ...
                speed_limit = min(speed_limit,
                                  math.sqrt(max(0, (distance + e.remaining_distance - self.stop_before_end_of_the_line) / self.slow_down_distance)))
            ...
            if (distance - self.stop_before_end_of_the_line) < self.slow_down_distance:
                if position.piece.claimed_by == train:
                    ...
                    speed_limit = min(speed_limit,
                                      math.sqrt(max(0, (distance - self.stop_before_end_of_the_line) / self.slow_down_distance)))
                elif train.speed:
            ...
            if train not in position.piece.reservations or position.piece.reservations[train]['distance'] > distance:
                position.piece.reservations[train] = {'distance': distance,
            ...
            other_reservations = [reservation
                                  ...
                                  if other_train != train and reservation['anchor_name'] != position.anchor_name]
            if can_hide_behind_decision_point and any(not reservation['can_hide_behind_decision_point']
        ...
        rear_position, _ = train.rear_position.reversed().next_piece(use_branch_decisions=True)
